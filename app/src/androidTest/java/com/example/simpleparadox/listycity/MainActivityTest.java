...
public class MainActivityTest{
    ...
    public void checkListItemClicks(){
        ...
        solo.clickOnButton("CONFIRM");
        solo.clearEditText((EditText) solo.getView(R.id.editText_name));
        assertTrue(solo.waitForText("dhaka", 1, 1000));

        solo.assertCurrentActivity("Wrong Activity", MainActivity.class);
        solo.clickOnButton("ADD CITY");
        solo.enterText((EditText) solo.getView(R.id.editText_name), "Sylhet");
        solo.clickOnButton("CONFIRM");
        solo.clearEditText((EditText) solo.getView(R.id.editText_name));
        assertTrue(solo.waitForText("Sylhet", 1, 1000));


        String city2 = (String) listView.getItemAtPosition(0);
        assertEquals("dhaka", city2);
